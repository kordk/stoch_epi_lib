...
Match gene expression and methylation state from the GTP Study:
- Mt: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE72680
- Gx (PMID 21536970): https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE58137

Binding site to target (Phi) data are from the eCPG predictions generated using the MESA study data (PMID 29229604).

| Filename | Description |
| :----- | :---------- |
| demo/29914364.MESA.eCpG.txt | From study XXX and downloaded from GEO |
| demo/GSE72680_beta_values.txt.gz | From GEO XXX and downloaded from GEO |

Many promoter regions contain multiple methylation loci. Aggregate the epigenetic loci together and summarize as the average methylation score. Merge the GTP methylation data into the MESA regions (Promoter) and create the Phi (binding site to target) data (MESA eCpG):

<pre>
aggregateAssayToRegion.py \
     -s average \
     -a demo/29914364.MESA.eCpG.txt \
     -d demo/GSE72680_beta_values.txt.gz
</pre>

| Filename | Description |
| :----- | :---------- |
| demo/GSE58137_Raw_119_samplesremoved.csv | From study XXX and downloaded from GEO |
| demo/ht12_v3.txt | From GEO XXX and downloaded from GEO |
| demo/sym2entrex.txt | From GEO XXX and downloaded from GEO |
| demo/regionAggregateData.csv | aggregated methylation scores generated by aggregateAssayToRegion.py in the previous step |
| demo/GSE72680.mid2pid.txt | Mapping between the IDs of the gene expression and methylation datasets |
